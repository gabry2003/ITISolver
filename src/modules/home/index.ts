...
export function calcolaRisultato(funzione: string, parlato: string) {
    ...
    if (coefficienteAngolare) {
        ...
        if (coefficienteAngolare.angoloAcuto) {
            descrizioneFunzione += mathsolver.toLatex(`\\text{La retta forma un angolo acuto con il semiasse positivo delle x}`);
        }

        if (coefficienteAngolare.angoloRetto) {
            descrizioneFunzione += mathsolver.toLatex(`\\text{La retta è parallela all'asse x}`);
        }

        if (coefficienteAngolare.angoloOttuso) {
            descrizioneFunzione += mathsolver.toLatex(`\\text{La retta forma un angolo ottuso con il semiasse positivo delle x}`);
        }
    }
